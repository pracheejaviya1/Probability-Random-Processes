...
\begin{center}
    \includegraphics[scale=0.5]{Reliability.png}
\end{center}
\newpage
\item \textbf{New figure}
\begin{center}
    \includegraphics[scale=0.8]{reli.PNG}
\end{center}

\item \textbf{New Inference}\\
We proposed a method for allocating a system reliability requirement with a given confidence level.The proposed method formalizes the allocation problem as an optimization issue. It is a general method and can be applied to systems with serial, parallel, and complex reliability
configurations. All the constraints such as the cost, time, and other factors can be easily handled by the proposed method.
